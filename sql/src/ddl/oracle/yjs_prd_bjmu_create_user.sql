...
------外键查询
select *
  from user_constraints cons_f
 where cons_f.r_constraint_name in
       (select cons_p.constraint_name
          from user_constraints cons_p
         where cons_p.constraint_type = 'P'
           and table_name = '表名字')
   and cons_f.constraint_type = 'R'
